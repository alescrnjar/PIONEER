...
def tokens_to_one_hot_TT(tokens, one_hot_dim):
    """
    Converts an L-vector of integers in the range [0, D] to an L x D one-hot
    encoding. The value `D` must be provided as `one_hot_dim`. A token of D
    means the one-hot encoding is all 0s.
    """
    #identity = nn.Identity(one_hot_dim + 1)[:, :-1].to(device)  # Last row is all 0s
...
def dinuc_shuffle_AC(seq, num_shufs=None, rng=None):
    ...
    elif type(seq) is torch.Tensor and len(seq.shape) == 2:
        seq_len, one_hot_dim = seq.shape
        arr = one_hot_to_tokens_TT(seq)
    else:
    ...
    # original characters
    if type(seq) is torch.Tensor:
        chars, tokens = torch.unique(arr, return_inverse=True)
    else:
    ...
    for t in range(len(chars)):
        ...
        if type(seq) is torch.Tensor:
            inds = torch.where(mask)[0] #.to(device)
        else:
    ...
    for i in range(num_shufs if num_shufs else 1):
        ...
        elif type(seq) is torch.Tensor:
            all_results[i] = tokens_to_one_hot_TT(chars[result], one_hot_dim)
        else:
