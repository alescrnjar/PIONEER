...
def make_mutants_Jack(originals,mutation_numbers,
                      sequence_length,
                      seed,
                      ):
    ...
    positions = np.array([rng.choice(sequence_length,size  = mutation_numbers, replace =  False) for i in range(n_to_make)])
    ...
    for i_nt in range(pvals.shape[0]): #QUIQUINONURG this is not optimal probably.
        ...
        # b[2]=2
        # a[:]*=b
    mutations = rng.multinomial(1, pvals, size=(n_to_make*mutation_numbers) )
...
class RandomMutation_up_to_ratio(AugmentBase):
    def __init__(self, mutate_frac=0.05):
        self.mutate_frac = mutate_frac
    ...
    def __call__(self, x):
        ...
        # make a copy of the batch of sequences
        x_aug = torch.clone(x)
        for cycle in range(num_mutations):
            num__mutations=1
            # randomly determine the indices to apply mutations
            mutation_inds = torch.argsort(torch.rand(N,L))[:, :num__mutations] # see <https://discuss.pytorch.org/t/torch-equivalent-of-numpy-random-choice/16146>0

            # create random DNA (to serve as random mutations)
            a = torch.eye(A)
            p = torch.tensor([1/A for _ in range(A)])
            mutations = torch.stack([a[p.multinomial(num__mutations, replacement=True)].transpose(0,1) for _ in range(N)]).to(x_aug.device)
...
class RandomMutation_up_to_ratio_Jack(AugmentBase):
    def __init__(self, mutate_frac=0.05):
        self.mutate_frac = mutate_frac
    ...
    #def mutate_randomly(self, n_to_make, x_source, mutation_number, cycles=1, ranker=None):
        ...
        #print("____________________________________ INSIDE RandomMutation_up_to_ratio_Jack")
        N, A, L = x.shape
        n_to_make=N
        x_source=x
        cycles=int(L*self.mutate_frac)
        #print(f"{cycles=}")
        mutation_number=1
        #seed=41 #QUIQUIURG 
        seed=42 #QUIQUIURG 
        ...
        for iround in range(nrounds): #NTOMAKELARGER
            ...
            originals = x #self.select_from_source(n2make, x_source, replace = False) #NTOMAKELARGER                
            to_mutate = np.array(originals)
            or_s = to_mutate
            for c in range(cycles):
                #to_mutate_1 = make_mutants_Jack(to_mutate,mutation_number,  L, seed)
                to_mutate_1 = make_mutants_Jack(to_mutate,mutation_number,  L, seed+c)
                #print(f"{(to_mutate==or_s).all()=} {(to_mutate==to_mutate_1).all()=}")
                to_mutate = to_mutate_1
