...
class PL_mpra(pl.LightningModule):
    def __init__(self,
                 batch_size=100, #128,  # original: 128, #20, #50, #100, #128,
        ...
        super().__init__()
        self.scale = scale
        #self.model = mpra(output_dim=3) #ANIRBAN  # , initialization=initialization, initialize_dense=initialize_dense) #.to(device) #goodold
        self.model = mpra(output_dim=1)  # , initialization=initialization, initialize_dense=initialize_dense) #.to(device) #goodold
        self.name = 'mpra'
        self.task_type='single_task_regression' #ANIRBAN: commented out
        ...
        if initial_ds:
            ...
            self.y_train = torch.tensor(np.array(data['y_train']).astype(np.float32))  # .unsqueeze(1)
            self.X_test = torch.tensor(np.array(data['x_test']).astype(np.float32)).permute(0,2,1)
            self.y_test = torch.tensor(np.array(data['y_test']).astype(np.float32))  # .unsqueeze(1)
            self.X_valid = torch.tensor(np.array(data['x_valid']).astype(np.float32)).permute(0,2,1)
            self.y_valid = torch.tensor(np.array(data['y_valid']).astype(np.float32))  # .unsqueeze(1)                                                     #DSRR
...
def training_with_PL(chosen_model, chosen_dataset,
    ...
    #model = eval("PL_" + chosen_model + "(input_h5_file='./" + chosen_dataset + ".h5', initial_ds=True)")  # ANIRBAN
    ...
    #os.system('mv ./' + ckptfile + '-v1.ckpt ./' + ckptfile + '.ckpt') #ANIRBAN
...
if __name__ == '__main__':
    ...
    pairlist = [
        ['mpra', 'LentiMPRA_processed_for_dal'],
    ...
    for pair in pairlist:
        ...
        metrics = training_with_PL(chosen_model, chosen_dataset, initial_test=False, mcdropout_test=False, verbose=False,
                                   wanted_wandb=False)
